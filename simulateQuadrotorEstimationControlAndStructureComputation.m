function [Q, Est, Ms] = simulateQuadrotorEstimationControlAndStructureComputation(R,S,P)
...
rxArray = cell(Nf, N-1);
RCIArrayTrue = cell(Nf, N-1);
rcArrayTrue = cell(Nf, N-1);
RCIArrayEst = cell(Nf, N-1);
rcArrayEst = cell(Nf, N-1); % This is inefficient because we are already saving these states and returning them, but the oversampling complicates things and this is good enough
...
for kk=1:N-1
  ...
  for ii=1:Nf
    ...
    if(isempty(rx))
      ...
      M.rxMat(ii,:) = rx';
      rxArray{ii, kk} = rx;
      RCIArrayTrue{ii, kk} = P.sensorParams.RCB*statek.RBI; % Compute the RCI matrix and store it
      rcArrayTrue{ii, kk} = statek.rI + statek.RBI.'*P.sensorParams.rocB; % Compute the camera origin and store it 
      if kk>1
          RCIArrayEst{ii, kk} = P.sensorParams.RCB*E.statek.RBI; % Compute the RCI matrix and store it
          rcArrayEst{ii, kk} = E.statek.rI + E.statek.RBI.'*P.sensorParams.rocB; % Compute the camera origin and store it 
      end
    end
  end
...
% The camera measurements for structure computation
Ms.rxArray = rxArray;
Ms.RCIArrayTrue = RCIArrayTrue;
Ms.RCIArrayEst = RCIArrayEst;
Ms.rcArrayTrue = rcArrayTrue;
Ms.rcArrayEst = rcArrayEst;
end
