...
Fo = (NB(2) + Ft*rp) / (2*rp);
Fi = Ft - Fo;
omega_rotor = ((Ft*0.25) / (qp.kF(1)))^0.5;
...
S.state0.e = [0 0 0]'; % Make sure this is expected orientation
% Initial velocity of body with respect to I, expressed in I, in m/s
S.state0.v = [0 0 0]'; % Need to make sure this is in the proper direction according to omegaB
% Initial angular rate of body with respect to I, expressed in B, in rad/s
S.state0.omegaB = zeros(3, 1); % This should be directly transferable 
...
S.omegaMat = omega_rotor*ones(N-1,4); % No idea if this is correct
% Oversampling factor
S.oversampFact = 10;
